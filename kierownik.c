/* kierownik.c - zarządzanie liczbą kas i sygnały (sygnał1/2/3) */
...
void* kierownik(void* arg)
{
    ...
    while(!przerwanie){
        ...
        if(potrzebne != czynne_kasy){
            snprintf(buf, sizeof(buf), "Kierownik: zmiana liczby kas z %d na %d (kolejka=%d total=%d baza=%d)",
                     czynne_kasy, potrzebne, q, total_queue, baza);
            loguj(buf);
            open_close_count++;
        }
